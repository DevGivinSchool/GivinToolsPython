# TODO Когда пользователь разблокируется (статус с B на P) - ему нужно пароль удалять 55
# TODO Для таких пользователей нужно вносить участника в FIO_ENG и в платежах тоже добавить такие поля -
#      Payment 646 for participant None created - (1305, 'XAYDAROVA FIRUZA')
#      c:\!SAVE\Хайдарова Фирюза.txt
...
#      нужно писать письмо админу чтобы разблокировал учётку zoom
...
# TODO (Для GetCouse такого не нужно , т.к. там все платежи за ДШ, а для paykeeper достаточно вывести PAYMENT чтобы понять что это за письмо)
#      Если body_type = html тогда преобразовывать его в простой текст перед записью в лог
...
# TODO Фридман Алисия написала FRIDMAN ALISIA.  Здесь можно распознать что написано по английски и сделать обратное
#  преобразование и поискать по русски + сделать таблицу вариантов написанияи имен, фамилий,
#  так же учесть вариант что может быть несколько почт и даже телеграм.
